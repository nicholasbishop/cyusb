...
/* Maximum length of a string read from the Configuration file (/etc/cyusb.conf)
 * for the library. */
...
/* Maximum size of EZ-USB FX3 firmware binary. Limited by amount of RAM
 * available. */
#define FX3_MAX_FW_SIZE (524288)

static struct cydev
    cydev[MAXDEVICES]; /* List of devices of interest that are connected. */
static int nid;        /* Number of Interesting Devices. */
static libusb_device** list; /* libusb device list used by the cyusb library. */

/*
   struct VPD
   Used to store information about the devices of interest listed in
   /etc/cyusb.conf
 */
struct VPD {
...
static unsigned int checksum =
    0; /* Checksum calculated on the Cypress firmware binary. */

/* The following variables are used by the cyusb_linux application. */
char pidfile[MAX_FILEPATH_LENGTH]; /* Full path to the PID file specified in
                                      /etc/cyusb.conf */
char logfile[MAX_FILEPATH_LENGTH]; /* Full path to the LOG file specified in
                                      /etc/cyusb.conf */
...
static void parse_configfile(void) {
  ...
  while (fgets(buf, MAX_CFG_LINE_LENGTH, inp)) {
    ...
    if (buf[0] == '\n')
      continue;
    if (isempty(buf,
                strlen(buf))) /* Any blank line is also ignored		*/
      continue;
    ...
    if (!strcmp(cp1, "LogFile")) {
      ...
      while (fgets(buf, MAX_CFG_LINE_LENGTH, inp)) {
        ...
        vpd[maxdevices].desc[MAX_STR_LEN - 1] =
            '\0'; /* Make sure of NULL-termination. */
...
int cyusb_open(void) {
  ...
  if (fd1 < 0) {
    ...
    close(fd1);
    parse_configfile(); /* Parse the file and store information inside exported
                           data structures */
  }
...
/* cyusb_error:
   Print verbose information about the error returned by the cyusb API. These
   are essentially descriptions of status values defined as part of the libusb
   library.
 */
...
/* cyusb_get_max_iso_packet_size:
   Calculate the maximum packet size which a specific isochronous endpoint is
   capable of sending or receiving in the duration of 1 microframe.
 */
...
/* cyusb_claim_interface:
   Claim the specified USB device interface, so that cyusb/libusb APIs can be
   used for transfers on the corresponding endpoints.
 */
 ...
 */
int cyusb_set_interface_alt_setting(cyusb_handle* h,
                                    int interface,
                                    int altsetting) {
...
/* cyusb_kernel_driver_active:
   Check whether a Kernel driver is currently bound to the specified USB device
   and interface. The cyusb/libusb APIs cannot be used on an interface while a
...
/* cyusb_detach_kernel_driver:
   Detach the kernel driver from the specified USB device and interface, so that
 ...
 */
int cyusb_get_active_config_descriptor(
    cyusb_handle* h,
...
/* cyusb_get_config_descriptor:
   Get the configuration descriptor at index config_index, for the specified USB
   device.
 */
int cyusb_get_config_descriptor(cyusb_handle* h,
...
/* cyusb_get_config_descriptor_by_value:
   Get the configuration descriptor with value bConfigurationValue for the
   specified USB device.
 */
int cyusb_get_config_descriptor_by_value(
    cyusb_handle* h,
    unsigned char bConfigurationValue,
    struct usb_config_descriptor** config) {
  cyusb_device* tdev = libusb_get_device(h);
  return (libusb_get_config_descriptor_by_value(
      tdev, bConfigurationValue, (struct libusb_config_descriptor**)config));
}
 ...
 */
int cyusb_get_string_descriptor_ascii(cyusb_handle* h,
                                      unsigned char index,
                                      unsigned char* data,
                                      int length) {
  cyusb_device* tdev = libusb_get_device(h);
 ...
 */
int cyusb_get_descriptor(cyusb_handle* h,
 ...
 */
int cyusb_get_string_descriptor(cyusb_handle* h,
                                unsigned char desc_index,
...
/* cyusb_control_transfer:
   Perform the desired USB control transfer (can be read or write based on the
   bmRequestType value).
 */
int cyusb_control_transfer(cyusb_handle* h,
  ...
  return (libusb_control_transfer(h, bmRequestType, bRequest, wValue, wIndex,
 ...
 */
int cyusb_control_read(cyusb_handle* h,
                       unsigned char bmRequestType,
                       unsigned char bRequest,
                       unsigned short wValue,
                       unsigned short wIndex,
  ...
  return (libusb_control_transfer(h, bmRequestType | 0x80, bRequest, wValue,
                                  wIndex, data, wLength, timeout));
}

/* cyusb_control_write:
   Perform a write transfer on the USB control endpoint. This can be a zero byte
   (no data) transfer as well.
 */
int cyusb_control_write(cyusb_handle* h,
                        unsigned char bmRequestType,
                        unsigned char bRequest,
                        unsigned short wValue,
                        unsigned short wIndex,
  ...
  return (libusb_control_transfer(h, bmRequestType & 0x7F, bRequest, wValue,
                                  wIndex, data, wLength, timeout));
}
 ...
 */
int cyusb_bulk_transfer(cyusb_handle* h,
                        ...
                        int timeout) {
  return (
      libusb_bulk_transfer(h, endpoint, data, length, transferred, timeout));
}

/* cyusb_interrupt_transfer:
   Perform a data transfer on a USB interrupt endpoint.
 */
int cyusb_interrupt_transfer(cyusb_handle* h,
                             unsigned char endpoint,
                             unsigned char* data,
                             int length,
                             int* transferred,
                             unsigned int timeout) {
  return (libusb_interrupt_transfer(h, endpoint, data, length, transferred,
                                    timeout));
}
 ...
 */
int cyusb_download_fx2(cyusb_handle* h,
                       char* filename,
  ...
  /* Place the FX2/FX2LP CPU in reset, so that the vendor commands can be
  ...
  while (fgets(buf, 256, fp) != NULL) {
    ...
    r = cyusb_control_transfer(h, 0x40, vendor_command, address, 0x00, dbuf,
                               num_bytes, 1000);
    if (!r) {
      printf("Error in control_transfer\n");
      free(dbuf);
...
/* control_transfer:
   Internal function that issues the vendor command that incrementally loads
   firmware segments to the Cypress FX3 device RAM.
 */
static void control_transfer(cyusb_handle* h,
  ...
  while (balance > 0) {
    if (balance > 4096)
      ...
      b = balance;
    r = cyusb_control_transfer(h, 0x40, 0xA0, (address & 0x0000ffff),
                               address >> 16, &dbuf[index], b, 1000);
    if (r != b) {
      printf("Error in control_transfer\n");
    }
    address += b;
...
int cyusb_download_fx3(cyusb_handle* h, const char* filename) {
  ...
  sleep(1);
  r = cyusb_control_transfer(h, 0x40, 0xA0, (program_entry & 0x0000ffff),
                             program_entry >> 16, NULL, 0, 1000);
  if (r) {
