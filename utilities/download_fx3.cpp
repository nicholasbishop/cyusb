 ...
 * Description          : Downloads FX3 firmware to RAM, I2C EEPROM or SPI
 * Flash.
 */
...
#define MAX_WRITE_SIZE \
  (2 *                 \
   1024)  // Max. size of data that can be written through one vendor command.

#define I2C_PAGE_SIZE (64)  // Page size for I2C EEPROM.
#define I2C_SLAVE_SIZE \
  (64 * 1024)  // Max. size of data that can fit on one EEPROM address.

#define SPI_PAGE_SIZE (256)          // Page size for SPI flash memory.
#define SPI_SECTOR_SIZE (64 * 1024)  // Sector size for SPI flash memory.

#define VENDORCMD_TIMEOUT \
  (5000)  // Timeout (in milliseconds) for each vendor command.
#define GETHANDLE_TIMEOUT \
  (5)  // Timeout (in seconds) for getting a FX3 flash programmer handle.

/* Utility macros. */
#define ROUND_UP(n, v) \
  ((((n) + ((v)-1)) / (v)) * (v))  // Round n upto a multiple of v.
#define GET_LSW(v)    \
  ((unsigned short)(( \
      v)&0xFFFF))  // Get Least Significant Word part of an integer.
#define GET_MSW(v)         \
  ((unsigned short)((v) >> \
                    16))  // Get Most Significant Word part of an integer.
...
/* Array representing physical size of EEPROM corresponding to each size
 * encoding. */
...
static int fx3_ram_write(cyusb_handle* h,
                         unsigned char* buf,
  ...
  while (len > 0) {
    ...
    r = cyusb_control_transfer(h, 0x40, 0xA0, GET_LSW(ramAddress),
                               GET_MSW(ramAddress), &buf[index], size,
                               VENDORCMD_TIMEOUT);
    if (r != size) {
...
static int read_firmware_image(const char* filename,
                               unsigned char* buf,
                               int* romsize,
                               int* filesize) {
  int fd;
...
static int fx3_usbboot_download(cyusb_handle* h, const char* filename) {
  unsigned char* fwBuf;
  unsigned int* data_p;
  ...
  if (fwBuf == 0) {
    fprintf(stderr,
            "Error: Failed to allocate buffer to store firmware binary\n");
    return -1;
  }
  ...
  // Run through each section of code, and use vendor commands to download them
  // to RAM.
  index = 4;
  checksum = 0;
  while (index < filesize) {
    ...
    if (length != 0) {
      ...
      r = cyusb_control_transfer(h, 0x40, 0xA0, GET_LSW(address),
                                 GET_MSW(address), NULL, 0, VENDORCMD_TIMEOUT);
      if (r != 0)
...
static int check_fx3_flashprog(cyusb_handle* handle) {
  ...
  r = cyusb_control_transfer(handle, 0xC0, 0xB0, 0, 0, (unsigned char*)local, 8,
                             VENDORCMD_TIMEOUT);
...
static int get_fx3_prog_handle(cyusb_handle** h) {
  char *progfile_p, *tmp;
  cyusb_handle* handle;
  int i, j, r;
  struct stat filestat;
...
static int fx3_i2c_write(cyusb_handle* h,
                         unsigned char* buf,
                         int devAddr,
                         int start,
                         int len) {
  ...
  while (len > 0) {
    size = (len > MAX_WRITE_SIZE) ? MAX_WRITE_SIZE : len;
    r = cyusb_control_transfer(h, 0x40, 0xBA, devAddr, address, &buf[index],
                               size, VENDORCMD_TIMEOUT);
    if (r != size) {
...
static int fx3_i2c_read_verify(cyusb_handle* h,
  ...
  while (len > 0) {
    size = (len > MAX_WRITE_SIZE) ? MAX_WRITE_SIZE : len;
    r = cyusb_control_transfer(h, 0xC0, 0xBB, devAddr, address, tmpBuf, size,
                               VENDORCMD_TIMEOUT);
    if (r != size) {
...
int fx3_i2cboot_download(cyusb_handle* h, const char* filename) {
  ...
  if (read_firmware_image(filename, fwBuf, &romsize, &filesize)) {
    fprintf(stderr,
            "Error: File %s does not contain valid FX3 firmware image\n",
            filename);
    free(fwBuf);
    return -2;
  }
  ...
  while (filesize != 0) {
    ...
    if (size > I2C_SLAVE_SIZE) {
      ...
      if (r == 0) {
        ...
        r = fx3_i2c_write(h, fwBuf, address + 4, offset + I2C_SLAVE_SIZE,
                          size - I2C_SLAVE_SIZE);
        if (r == 0) {
...
static int fx3_spi_write(cyusb_handle* h, unsigned char* buf, int len) {
  int r = 0;
  int index = 0;
  int size;
  ...
  while (len > 0) {
    size = (len > MAX_WRITE_SIZE) ? MAX_WRITE_SIZE : len;
    r = cyusb_control_transfer(h, 0x40, 0xC2, 0, page_address, &buf[index],
                               size, VENDORCMD_TIMEOUT);
    if (r != size) {
...
static int fx3_spi_erase_sector(cyusb_handle* h, unsigned short nsector) {
  unsigned char stat;
  int timeout = 10;
  int r;

  r = cyusb_control_transfer(h, 0x40, 0xC4, 1, nsector, NULL, 0,
                             VENDORCMD_TIMEOUT);
  if (r != 0) {
  ...
  do {
    r = cyusb_control_transfer(h, 0xC0, 0xC4, 0, 0, &stat, 1,
                               VENDORCMD_TIMEOUT);
    if (r != 1) {
...
void print_usage_info(const char* arg0) {
  ...
  printf(
      "\t%s -t <target> -i <img filename>: Program firmware binary from <img "
      "filename>\n",
      arg0);
  printf("\t\tto <target>\n");
